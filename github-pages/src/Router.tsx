...
function SurvivalMethodPage() {
  return <main className="survival-method-page">
    ...
    <section className="survival-method-research"><p>BOUNDED AUTONOMOUS DISCOVERY · NEW POLICY 4 STUDIES</p><h2>A problem, a prediction, a physical test.</h2><p>New studies compare protection problems using an agent’s own observations and forecast needs. Staying, moving, gathering, resting and trying an arrangement compete on estimated survival benefit and complete-plan costs. A raised arrangement can create a support subgoal. A test is optional: uncertainty must matter to a later decision, and urgent needs take priority.</p><p>Predictions are recorded before paid operations. Local protection readings and load tests update small, context-dependent estimates; conflicting evidence can change or end a plan. A passed load is evidence about that load, not an exact breaking strength. Protection is a supplied local measurement, not proof that one part caused the result. Executed, measured sequences can become uncertain procedures that are adapted and performed again on new objects.</p><p>We supply senses, physiology, rough material and geometry expectations, primitive operations, world rules and finite search budgets. Agents learn contextual corrections and useful procedures within that bounded system. They do not receive hidden world coefficients, another agent’s private memories or hypothetical world-evaluator answers. No curiosity reward, named-invention unlock, API or external model is used. This first domain does not establish general technological invention or better survival in every environment.</p></section>
    <section className="survival-method-research"><p>PRESERVED POLICY 3 STUDIES</p><h2>Try an arrangement. Measure what happened.</h2><p>Policy 3 agents can propose projects to reduce future exposure, gather materials, shape and position parts, connect or modify observed objects, and measure the results. They compare predicted survival benefit with effort, uncertainty, and ordinary alternatives such as resting or finding supplies. Urgent needs can interrupt a project. Failed parts remain in the world.</p><p>Wood, stone, fiber and clay have simplified mass, strength, insulation, porosity and heat properties. Geometry and connections determine support and protection—not a building name. Older policy 3 tests include a modeled counterfactual for protection; new policy 4 keeps that diagnostic out of agent knowledge and uses local readings instead. Each agent keeps its own evidence and fallible estimates and can exchange a reported result with consent. A single successful trial is not proof of a general invention.</p><p>The action language and search budget are supplied by us. This is not full chemistry, rigid-body physics, electricity or an unrestricted inventor. Older studies retain their policy family and sensing behavior; active policy-2/3 studies adopt survival and navigation corrections with a recorded update. Policy 1 remains frozen. Configure a new run to use policy 4; opening a saved study does not convert it.</p></section>
