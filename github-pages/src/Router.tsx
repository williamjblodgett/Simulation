...
const AUTONOMY_CYCLE = [
  ...
  ["04", "Learn", "Confirmed outcomes update expectations and persistent affective signals, so later choices can change without a preset personality class."],
] as const;
...
function SurvivalMethodPage() {
  return <main className="survival-method-page">
    ...
    <section className="survival-method-research"><p>PROTECTION, SUPPORT AND MATERIAL TRANSFORMATION · NEW POLICY 4 STUDIES</p><h2>Try a process. Measure what happened.</h2><p>An agent can recognize repeated work as a survival cost, form a goal to reduce it, gather a source-specific sample, prepare carbon-rich fuel, separate mineral-bearing rock, form a fired enclosure, attempt ore reduction, work a bloom and physically test a degrading tool. It may also abstain. Every intervention costs modeled time, energy and matter; unsuccessful heating leaves explicit remnants instead of refunding inputs.</p><p>The authoritative world—not the agent’s wish—calculates attained temperature from fuel, fuel quality, airflow, charge mass, enclosure condition and duration. Hidden ore composition controls possible yield. The agent receives only coarse observations, broad fallible priors and its own measurements. A supported procedure still pays every physical cost and can fail when conditions change.</p><p>Developers supply senses, physiology, action capabilities, broad mechanism priors and world rules. Agents learn contextual effects, operating ranges and procedures within those limits; they are not blank minds. Their private policy input excludes the world seed, exact chemistry, another agent’s private memory and observer diagnostics. No API, external model, curiosity reward or named-invention unlock is used.</p></section>
    <section className="survival-method-research"><p>RESEARCH IS AN ACTION, NOT AN UNLOCK BUTTON</p><h2>A foundation for discovery, not automatic modern technology.</h2><p>The world implements simplified heat transfer, combustion and pyrolysis, source-specific separation, ceramic firing, refractory containment, copper and iron reduction, hot working, yield, waste and tool wear. Predictions, physical interventions, measurements and later decisions remain separate records.</p><p>The model accounts for normalized fuel and ore mass, estimated degrees Celsius, ore grade, reduction fraction, solid yield, slag, exhaust, hearth wear, tool durability, modeled minutes and physiological energy. It is not complete thermodynamics or chemistry. The 28 geological families are possible raw inputs, not pure elements, and most still lack executable processing rules.</p><p>There are no powered machines, precision metrology, electrical circuits or semiconductor fabrication yet. More modeled time does not automatically produce technology, and survival can rationally remain more important than experimentation.</p></section>
    ...
    <section className="survival-method-boundary"><h2>They learn arrangements, not a picture of a house.</h2><p>Agents start with basic abilities to handle materials and imperfect expectations, not a catalog of buildings. They reason about shapes they have observed, look for clear supported positions and an approach, obtain missing materials, and return to unfinished work. Urgent needs can interrupt construction. Repeated failures can make them revise or abandon a project.</p><p>Resting near an arrangement creates an experience record. An agent can later choose to return there when it expects protection to help. This is distinct from a controlled material test: a warmer night does not prove a structure caused the improvement. Select a physical part to inspect its maker, project, measurements and recent use. Dashed outlines show proposals, not completed buildings. The observer cannot approve a design or tell an agent to build it.</p><h2>Autonomous does not mean conscious.</h2><p>These are deterministic simulation agents with bounded perception, planning, uncertainty, memory, outcome learning and adaptive control signals—not sentient beings and not hidden chatbots. Caution, frustration, confidence, inquiry and social need are numeric states derived from experienced conditions. They reweight decisions but do not create consciousness, a personality class or hidden chain-of-thought.</p><p>New studies are open-ended. You can add an independent agent at any time, up to five living agents—even after extinction. Additions are logged and do not restart the world. Closing every Simulation tab pauses local advancement rather than fabricating unseen history.</p><p>Death remains permanent. When optional continuity is enabled, an experienced agent may reserve supplies for a successor or sponsor one after personally observing a death. It can decline or wait. The new life has lineage but no inherited memories, research, personality or role.</p><p>A funded plan admits one new life only after its predecessor dies and a slot is vacant. This is an abstract admission model, not biological reproduction or resurrection. Preserved finite studies still honor their recorded end, while completed records remain final.</p><p>Preserved active policy-2/3 studies adopt survival and navigation corrections with a recorded update. Paused and ended records are not rewritten; material knowledge and adaptive state remain explicit new-study options.</p><a href="#/planet">Open the preserved planetary study</a></section>
  </main>;
}
