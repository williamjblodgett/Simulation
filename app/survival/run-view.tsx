...
export function RunView({ world, storageStatus, onStart, methodHref = "/about", planetHref = "/planet" }: RunViewProps) {
  ...
  return <section ref={screenRef} className={`${styles.screenView} ${setupOpen ? styles.setupScreen : ""}`} aria-labelledby="run-heading">
    ...
    <section className={styles.runStatus} hidden={setupOpen}>
      ...
      <p className={styles.inlineNotice}>{world.policyVersion===3?"Physical construction study · agents use private observations to find clear positions, obtain prerequisites, retain interrupted work and learn from use. Existing records stay intact.":"Preserved earlier study · this run does not use physical construction. Configure a new run below to observe the updated agents; this study will remain in your device archive."}</p>
