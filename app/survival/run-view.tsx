...
import { DevelopmentWorldRecord } from "./development-record";
...
export function RunView({ world, storageStatus, onStart, methodHref = "/about", planetHref = "/planet" }: RunViewProps) {
  ...
  const [discoveryObjective,setDiscoveryObjective]=useState(false);
  ...
  return <section ref={screenRef} className={`${styles.screenView} ${setupOpen ? styles.setupScreen : ""}`} aria-labelledby="run-heading">
    ...
    <section className={styles.runStatus} hidden={setupOpen}>
      ...
      {!world.development && world.materials && world.status!=="completed" ? <section className={styles.durationCorrection} aria-label="Expanded physical world"><div><strong>Continue with the expanded world</strong><p>Enable recurring goals, storage, cultivation and connected mechanisms for this study. Existing lives, materials and history remain. Agents receive no completed inventions.</p></div><button type="button" disabled={runtime.busy||enabling} onClick={async()=>{setEnabling(true);try{await runtime.enableExpandedWorld();setAddMessage("The expanded world is enabled. Agents can consider its possibilities as they observe and act.");}catch{setAddMessage("The change could not be saved; the current study is intact.");}finally{setEnabling(false);}}}>{enabling?"Saving…":"Enable expanded world"}</button></section>:null}
      <DevelopmentWorldRecord world={world}/>
    ...
    {setupOpen ? <section className={styles.newRun}>
      ...
      <p className={styles.inlineNotice}>A shared physical world with mineral processing, storage, cultivation, evolving structures, mechanical power, electrical storage and relay control. Agents decide which possibilities justify their cost. No invention or pace of advancement is guaranteed.</p>
      ...
      <div className={styles.setupGrid}>
        ...
        <label><span>Discovery motivation</span><select value={discoveryObjective?"discovery":"survival"} onChange={e=>setDiscoveryObjective(e.target.value==="discovery")}><option value="survival">Only when survival benefits</option><option value="discovery">Survival + bounded discovery</option></select><small>{discoveryObjective?"An additional goal: up to eight effort units per day for useful uncertainty, after urgent needs are secure.":"Agents may stop experimenting when their current strategy is adequate."}</small></label>
      </div>
