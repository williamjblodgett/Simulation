...
export function PhysicalRecord({agent,onInspect,onViewRecord}:{agent:SurvivalAgent;onInspect?(id:string):void;onViewRecord?(id:string):void}){
  const mind=agent.physicalMind;if(!mind)return null;
  if(agent.discovery && !mind.projects.length)return <DiscoveryRecord agent={agent} onViewRecord={onViewRecord}/>;
  return <>
    <DiscoveryRecord agent={agent} onViewRecord={onViewRecord}/>
