    ...
    <div className={styles.agentDirectory}>
      {(allLives ? world.agents : currentRecords).map((agent) => <article key={agent.id} data-selected={selectedId === agent.id} data-alive={agent.alive}>
        <button type="button" className={styles.agentDirectoryMain} onClick={() => onInspect(agent)}>
          ...
          <div className={styles.agentDirectoryIdentity}><span>{agent.label}{agent.slotGeneration > 1 ? ` · Entry ${agent.slotGeneration}` : ""} · {agent.alive ? "Living" : "Died"}</span><h2>{agent.name.startsWith("Agent ") ? "Unnamed" : agent.name}</h2><p>{activityLabel(agent)}</p><ConditionLine agent={agent} /></div>
