...
export function MaterialWorldRecord({ world }: { world: SurvivalRunState }) {
  ...
  return <details className={styles.materialRecord}><summary>Raw-material world · {feedstockKinds.length} finite mineral families</summary>
    ...
    {world.materials ? <p><strong>Processing mechanisms are executable.</strong> Agents can test carbon preparation, mineral separation, fired heat containment, copper/iron reduction, hot working, ceramics and degrading tools. Exact grades and operating thresholds remain hidden world truth. {world.development?"Processed batches can become actual mechanism or conductor components; installation never duplicates their mass. Simplified powered systems do not imply semiconductor fabrication or arbitrary modern technology.":"Expanded mechanisms are available through the explicit Run upgrade; this preserved study has not adopted them."}</p> : <p><strong>Modern technology is not executable in this preserved study.</strong> New studies provide additional material-process and component rules; this saved world keeps its original behavior.</p>}
    <dl className={styles.rowList}>{feedstockKinds.map(kind => {
      ...
      const components=(world.development?.components??[]).reduce((n,c)=>n+(c.feedstocks[kind]??0),0)+(world.development?.wasteFeedstocks[kind]??0);
      return <div key={kind}><dt>{GEOLOGICAL_FEEDSTOCKS[kind].label}<small>{GEOLOGICAL_FEEDSTOCKS[kind].potential}</small></dt><dd>{site?.quantity.toFixed(1) ?? "Unavailable"} in ground<br/><small>{held.carried.toFixed(1)} carried · {(held.embodied+components).toFixed(1)} in parts/remnants · {processed.toFixed(1)} processed/remnant</small></dd></div>;
    })}</dl>
