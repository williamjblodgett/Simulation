...
function fenceCheckpoint(checkpoint: SurvivalCheckpoint, survivalFence = false, discoveryFence = false, materialFence = false, knowledgeFence = false): SurvivalCheckpoint {
...
export async function commitCheckpoint(next: SurvivalCheckpoint, events: SurvivalEvent[], expectedRevision: number | null): Promise<boolean> {
  ...
  return new Promise((resolve, reject) => {
    ...
    read.onsuccess = () => {
      ...
      if (previous) checkpoints.put(fenceCheckpoint(previous, next.world.schemaVersion === 4, next.world.schemaVersion === 5, next.world.schemaVersion === 6, next.world.schemaVersion === 7), "last-good");
  ...
  return new Promise((resolve, reject) => {
    ...
    active.onsuccess = () => {
      ...
      backup.onsuccess = () => {
        ...
        recovered = fenceCheckpoint({ ...backup.result, missingBefore: Math.min(backup.result.world.eventWindow.totalEvents, Math.max(backup.result.missingBefore, knownCutoff)), revision: Math.max(backup.result.revision, Number.isSafeInteger(active.result?.revision) ? active.result.revision : 0) + 1, savedAt: Date.now() }, active.result?.world?.schemaVersion === 4, active.result?.world?.schemaVersion === 5, active.result?.world?.schemaVersion === 6, active.result?.world?.schemaVersion === 7);
