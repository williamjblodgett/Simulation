...
function ObservedRunExperience({ methodHref = "/about", planetHref = "/planet" }: { methodHref?: string; planetHref?: string }) {
  ...
  function inspectPart(id:string){
    const part=world?.physical?.parts.find(p=>p.id===id)??world?.development?.components.find(p=>p.id===id);
  ...
  return <main className={styles.shell} data-view={view}>
    ...
    <div className={styles.content}>
      <section className={styles.worldView} data-level={sheetLevel} aria-label="Live survival world" hidden={view !== "world"}>
        ...
        <div className={styles.habitatCaption}><span>Field observation / {String(world.seed).slice(-5)}</span><strong>Wooded basin</strong><small>{world.stats.livingAgents} living · {(world.physical?.parts.length ?? world.environment.structures.length)+(world.development?.components.length??0)} {world.physical ? "constructed parts" : "structures"}</small></div>
        ...
        {partsOpen?<section className={`${styles.miniMap} ${styles.constructionShelf}`} aria-label="Constructions"><header><h2>Constructions</h2><button aria-label="Close constructions" onClick={()=>setPartsOpen(false)}><X size={18}/></button></header><p>Solid forms are built. Blue dashed outlines are proposals, not completed work.</p>{world.physical?.parts.length?<div>{world.physical.parts.map(p=>{const r=constructionRecord(world,p.id)!;return <button key={p.id} onClick={()=>inspectPart(p.id)}><strong>{humanize(r.title)}</strong><span>{r.maker?.label} · {r.status} · part {p.id.replace("part-","")}</span></button>;})}</div>:!world.development?.components.length?<p>{world.physical?"No parts have been constructed yet. Agents choose whether a project is worth pursuing.":"This saved study uses the earlier building model. New runs use physical construction; your current study will stay archived."}</p>:null}{world.development?.components.length?<div>{world.development.components.map(c=><button key={c.id} onClick={()=>inspectPart(c.id)}><strong>{humanize(`${c.material} ${c.form}`)}</strong><span>{world.agents.find(a=>a.id===c.makerId)?.label??"Earlier life"} · {Math.round(c.condition*100)}% condition · {c.output>.001?"producing output":"no active output"}</span></button>)}</div>:null}</section>:null}
