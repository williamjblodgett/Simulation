...
export function DiscoveryEventRecord({ event }: { event: SurvivalEvent }) {
  ...
  const prediction = record(experiment?.prediction) ?? (experiment && typeof experiment.effect === "number" ? experiment : null);
  ...
  return <section aria-label="Decision and measurement evidence">
    ...
    {prediction ? <><h4>Before-action prediction</h4><p>{typeof experiment?.claim === "string" ? experiment.claim : "Recorded before this physical attempt."} Expected reading {number(prediction.mean ?? prediction.effect)}; uncertainty width {number(prediction.uncertainty)} (not a calibrated probability).</p></> : null}
    {measurement ? <><h4>Confirmed measurement</h4><p>{String(measurement.interpretation ?? measurement.summary ?? "Measured physical outcome.")} Reading {number(measurement.value)}.</p><p>{Array.isArray(measurement.confounds) ? measurement.confounds.map(String).join(" ") : String(measurement.conditions ?? "")}</p><small>Original evidence {String(measurement.originalId)} · {String(measurement.source)} · observer {String(measurement.originalObserverId ?? measurement.observerId ?? "Not recorded")}</small></> : null}
